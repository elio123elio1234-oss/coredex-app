...
// ERROR_MISC bits the firmware reports as "the right-leg drive is not doing its job".
#define ERROR_MISC_CMOR     0x01  // common-mode level out of range
#define ERROR_MISC_RLDRAIL  0x02  // RLD amplifier output near a rail
...
class ads1293
{
  public:
    ...
    static void Read_Data_Stream(byte* data, int length = 9);
    static void Disable_Start(void);
    static void Enable_Start(void);
    static byte Read_LOD_Status(void);
    static byte Read_Error_Misc(void);
};

#endif
// v3.0.0 — 9-byte data loop (three channels), ERROR_MISC accessor + bit names. Register map unchanged from v2.
