...
void ads1293::Init(bool* is_init)
{
    *is_init = false;
    ...
    Reg_Write(FLEX_CH2_CN, 0b00011001); // INP to IN3 and INN to IN1 (Lead II: LL-RA)
    Reg_Write(FLEX_PACE_CN, 0);
    Reg_Write(FLEX_VBAT_CN, 0);
    // Lead-Off Detection: DC mode, enable on IN1(RA), IN2(LA), IN3(LL)
    Reg_Write(LOD_CN, 0x00);          // DC lead-off, LOW threshold (most sensitive)
    Reg_Write(LOD_EN, 0x07);          // Enable LOD on IN1, IN2, IN3
    ...
    Reg_Write(AFE_SHDN_CN, 0b100100); // Shut down CH3 only (sigma-delta + digital), CH1+CH2 active
    ...
    Reg_Write(CH_CNFG, 0b00110000); // loop read-back mode, for CH1 + CH2
    Enable_Start(); 
    *is_init = true;
}
