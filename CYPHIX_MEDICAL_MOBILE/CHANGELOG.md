...
⚠️ **NATIVE REBUILD — there is no OTA for this.** An app icon is compiled into
the binary (iOS asset catalog, Android `res/mipmap`), never carried in a JS
bundle, so `app.json` goes 0.36.0 → **0.37.0** and it ships with
`npm run ship:rebuild`.

**That also decides what happens to v0.66.0's lead debug screen.** It was
published OTA to runtime 0.36.0; this binary is 0.37.0, so that update can
never reach it — and does not need to. The screen's code is in this bundle,
baked into the binary. Nothing is lost, but the OTA that is currently sitting
on the 0.36.0 channel becomes dead the moment build 10 is installed.

**Five files from one square artwork**, generated by `npm run icons`
(`scripts/make-icons.ps1`, new):

| file | what it is |
|---|---|
| `icon.png` | iOS + legacy Android, full bleed, **no alpha channel** |
| `android-icon-foreground.png` | the adaptive foreground |
| `android-icon-background.png` | the same colour field with the heart averaged out |
| `android-icon-monochrome.png` | Android 13+ themed icon |
| `favicon.png` | web |

- **No alpha on `icon.png` is a requirement, not a preference** — the App Store
  rejects an app icon that carries an alpha channel, and the artwork is opaque
  anyway, so it is written as 24-bit RGB.
- **The geometry that had to be checked:** an Android adaptive layer is 108dp
  and a launcher may crop anything outside the middle 72dp, so only **66.7 %**
  of the frame is guaranteed. The heart occupies ~54 %, so it survives a
  circular mask with margin; only the horizontal pulse line runs off the edge,
  which it does in the artwork too. Both masks were **rendered and looked at**
  before shipping — §6.4 applies to pictures as much as to code, and this is a
  class of bug that no typecheck can reach.
- ★ **The monochrome icon is DRAWN, not derived.** Android tints that layer
  flat and discards its colour, so thresholding the artwork gives a ragged
  blob. It is a filled heart (the classic parametric curve, sampled as a
  polygon) with the pulse trace punched out of it. **The punch is verified:**
  the script reads the icon's centre column and fails if the R spike has
  reached the heart's own notch — when those two gaps merge, the lobes read as
  two separate blobs instead of one heart. The first two attempts did exactly
  that, because the spike was sized against the *canvas* instead of against the
  *heart*; hence a measurement in the script rather than an opinion in a
  comment.
- `adaptiveIcon.backgroundColor`: `#FFFFFF` → **`#9971D8`**, the mean of the
  artwork's own field. It is only a fallback, but a white flash behind a purple
  icon is not this palette.
- **Weight:** `System.Drawing`'s PNG encoder takes no compression settings and
  picks a poor filter, so the 1024 px icon first came out **larger than the
  1254 px artwork it was scaled down from** (1.8 MB). `scripts/recompress-png.js`
  re-encodes losslessly with adaptive per-scanline filtering and drops the
  unused alpha channel: 4.1 MB → 1.9 MB across the set, same pixels.
- **The splash screen is untouched** and still carries the old mark. That is a
  separate decision, not an oversight — say the word and it follows the icon.

⚠️ **§6.4:** the icons were rendered, masked and inspected on this machine.
Nobody has seen them on a home screen, because no binary carrying them exists
yet.
