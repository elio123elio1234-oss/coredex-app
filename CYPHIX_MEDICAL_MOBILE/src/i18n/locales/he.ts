...
export const he: Record<TranslationKey, string> = {
  ...
  /* מכשירי ליד II כפול בלבד (קושחה v3+) — ראו את ההערה ב-en.ts. */
  limbRldWarning:
    'אלקטרודת הייחוס אינה נוגעת בעור, ולכן אי אפשר לסמוך על האות. ' +
    'הצמידו את המכשיר לעור עד שההודעה תיעלם.',
  limbSecondLegWarning:
    'האלקטרודה השנייה של הרגל אינה נוגעת בעור. ההקלטה תשתמש בעותק יחיד של ליד II.',
  ...
  /* ★ v0.62.0 — ראו את ההערה ב-en.ts. */
  /* ★ v0.63.0 — שורת העדכון. */
  setAboutUpdate: 'עדכון האפליקציה',
  setUpdCheck: 'בדיקת עדכון',
  setUpdChecking: 'בודק…',
  setUpdDownloading: 'מוריד…',
  setUpdCurrent: 'יש לך את הגרסה החדשה ביותר',
  setUpdReady: 'מוכן — הקש כדי להפעיל מחדש עכשיו',
  setUpdReadyDesc:
    'גרסה חדשה כבר נמצאת על הטלפון. היא הייתה מגיעה מעצמה בפעם הבאה שתפתח את האפליקציה.',
  setUpdError: 'לא הצלחתי לבדוק כרגע — הקש כדי לנסות שוב',
  setUpdDev: 'לא רלוונטי בבילד פיתוח',
  insSignatureTitle: 'טביעת האצבע של הלב שלך',
  /* ★ v0.64.0 — ראו את ההערה ב-en.ts. */
  insSignatureBody:
    'זו פעימת לב אחת, ממוצע של כל ההקלטות שעשית — לא ההקלטה האחרונה שלך.',
  insSignatureWhy:
    'המיצוע הוא כל העניין. מה שחוזר בכל הקלטה זה הלב שלך; מה שלא — יד רועדת, אלקטרודה רופפת, החשמל בחדר — מבטל את עצמו. ככל שתוסיף בדיקות, הצורה הזאת נעשית נקייה יותר ושלך יותר.',
  insSignatureUse:
    'מולה נמדד כל מה שמתחת. כל בדיקה חדשה מושווית לצורה הזאת, והמספרים של "בדרך כלל" הם מה שהיא אומרת שנורמלי אצלך — לא אצל אנשים בכלל.',
  insSignatureA11y: 'פותח הסבר על מה העקומה הזאת',
  insLeadsCaption: 'לידים · כמה הקלטות עומדות מאחורי כל אחד',
  ...
  /* מוצע רק להקלטה שיש בה שני עותקים של ליד II — ראו את ההערה ב-en.ts. */
  vtFusion: 'איחוד ליד II',
  vtFusionHint:
    'משלב את שני העותקים הנמדדים של ליד II לעקבה אחת שקטה יותר, ללא סינון תדרים. כבוי מציג את האלקטרודה הראשונה בלבד.',

  /* ── ליד II כפול: מה נעשה לליד II של ההקלטה הזאת ──
     ה-LRM (U+200E) סביב המספרים אינו קישוט: בלי עוגן משמאל-לימין, אלגוריתם
     הדו-כיווניות הופך את "26 → 9" ל-"9 → 26" — והחץ, שאינו תו משתקף, נשאר
     מצביע לכיוון ההפוך. שורה שאומרת שהרעש עלה היא בדיוק ההפך מהאמת. */
  fusionFused: 'ליד II אוחד משתי אלקטרודות · רעש ‎{from} → {to} µV‎',
  fusionDeclined: 'ליד II מבוסס על האלקטרודה הראשונה בלבד — {reason}',
  fusionOff: 'איחוד ליד II כבוי — מוצגת האלקטרודה הראשונה בלבד',
  fusionWhyNoCopy: 'לא הוקלט עותק שני',
  fusionWhyLength: 'שני העותקים באורך שונה',
  fusionWhyShort: 'ההקלטה קצרה מכדי להשוות ביניהם',
  fusionWhyNotFinite: 'העותק השני מכיל דגימות לא תקינות',
  fusionWhyFlat: 'אחד העותקים הוא קו שטוח',
  fusionWhyNotSameLead: 'שני העותקים אינם תואמים (כנראה שהאלקטרודה השנייה לא נגעה בעור)',
  ...
  /* התיבה — הועתק מהלוקאל של הווב כמו שהוא. */
  ...
  /* v0.61.0: ראו את ההערה ב-en.ts. */
  pdfPageStatistics: 'טבלאות קליניות ושונות קצב',
  ...
  pdfLeadMapCap: 'לידים I, II ו-III הם שלוש צלעות המשולש של איינטהובן, שנוצר מהאלקטרודות שעל שתי הידיים והרגל השמאלית. aVR, aVL ו-aVF מחושבים מלידים I ו-II הנמדדים. יחד הם רואים את הלב במישור החזיתי בלבד.',
  ...
  /* v0.61.0: ראו את ההערה ב-en.ts. */
  pdfProcTitle: 'כיצד עובדה ההקלטה הזאת',
  pdfProcBody:
    'כל ערוץ עובר תיקון קו-בסיס במסנן חציון כפול, סינון נַאץ' +
    ' ב-{notch} Hz בפאזה אפס (כך שאף מרווח אינו מוזז בזמן), והחלקה קלה — לפני שהוא מצויר או נמדד. לידים I ו-II נמדדים, ואילו III, aVR, aVL ו-aVF מחושבים מהם לפי משוואות איינטהובן וגולדברגר — אריתמטיקה מדויקת, לא אלקטרודות נוספות. כל רצועה מודפסת ב-25 mm/s וב-10 mm/mV עם פולס כיול של 1 mV, כך שאפשר לאמת את ההגבר בעין ולא לסמוך עליו.',
  /* ליד II כפול (קושחה v3+): מודפס מתחת לפסקה שלמעלה, ורק בהקלטה שיש בה שני
     עותקים של ליד II — ראו את ההערה ב-en.ts. ה-LRM סביב המספרים: כמו
     ב-fusionFused. */
  pdfFusionFused:
    'ליד II נמדד פעמיים, דרך שתי אלקטרודות נפרדות על רגל שמאל, ושני העותקים אוחדו לעקבה אחת לפני ש-III, aVR, aVL ו-aVF חושבו: העותק השקט יותר מקבל משקל גדול יותר, ומה שאינו חוזר מפעימה לפעימה עובר מיצוע — לעולם לא בתוך ה-QRS. שלב זה אינו מפעיל שום מסנן תדרים. רעש שאינו חוזר בליד II: ‎{from} → {to} µV‎.',
  pdfFusionDeclined:
    'ליד II נמדד פעמיים, דרך שתי אלקטרודות נפרדות על רגל שמאל, אך העותק השני לא שימש: {reason}. כל הלידים בדוח זה מבוססים על העותק הראשון בלבד.',
...
// v1.25.0 — עברית לליד II הכפול, מפתח מול מפתח עם en.ts: שתי הערות האלקטרודות
//           במסך המדידה, מתג האיחוד בצופה, השורה שאומרת מה האיחוד עשה (ואם
//           נדחה — למה), והפסקה בדוח המודפס. שתי מחרוזות PDF שאמרו "שני
//           ערוצים" נוסחו מחדש כדי שיישארו נכונות גם להקלטה של שלושה ערוצים.
// v1.24.0 — ההסבר על עקומת החתימה עובר מהמסך אל הגיליון שהכותרת פותחת,
//           ושם הוא נעשה מלא (ראו את ההערה ב-en.ts).
// v1.23.0 — עברית לשורת עדכון האפליקציה בעמוד "אודות".
// v1.22.0 — עברית לכותרת של עקומת החתימה ("טביעת האצבע של הלב שלך"),
//           לשורה שמסבירה ממה היא עשויה, ולכיתוב מעל שורת הלידים.
// v1.21.0 — עברית לפרובננס של עיבוד האות בדוח המודפס, ושם חדש לעמוד
//           הסטטיסטיקה (ראו את ההערה ב-en.ts).
