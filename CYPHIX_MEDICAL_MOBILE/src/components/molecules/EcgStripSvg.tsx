/* ==================================================================
   ...
   on true ECG grid paper. A port of the web molecule of the same name.
   ...
   10 mm/mV, so a caliper laid on the screen (or on the eventual print)
   means what it has always meant on paper.
...
export default function EcgStripSvg({
  ...
  /* Real ECG paper is pink-orange on white. Kept on the phone because the
     grid IS the ruler and it has to read as one. */
  const paper = dark ? '#12161F' : '#FFF8F6';
  const gridMinor = dark ? 'rgba(255,138,128,0.16)' : 'rgba(230,120,110,0.30)';
  const gridMajor = dark ? 'rgba(255,138,128,0.34)' : 'rgba(214,88,74,0.55)';
  const trace = dark ? '#E8EDF5' : '#0A2540';
  const marker = dark ? '#38BDF8' : '#2F6BD8';
  ...
  return (
    <View style={styles.strip}>
      <View style={styles.head}>
        ...
        <Text style={[styles.scale, { color: t.textTertiary }]}>
      ...
      >
        <Rect width={widthMm} height={heightMm} fill={paper} />

        {/* 1 mm minor grid, 5 mm major grid — real ECG paper. Explicit lines
            rather than a pattern fill, so nothing rasterises. */}
        <Path d={grid.minor} fill="none" stroke={gridMinor} strokeWidth={0.1} />
        <Path d={grid.major} fill="none" stroke={gridMajor} strokeWidth={0.2} />
        ...
        {rPeaks?.map((r) => {
          ...
          return (
            <Line key={r} x1={x} y1={0.6} x2={x} y2={2.2} stroke={marker} strokeWidth={0.25} />
          );
        })}
...
const styles = StyleSheet.create({
  ...
  scale: { fontSize: 9.5, fontWeight: '600', fontVariant: ['tabular-nums'] },
});

// v1.0.0 — Vector mm-paper lead strip: shared grid + path geometry, fixed
//          25 mm/s · 10 mm/mV, calibration pulse and R-peak ticks.
