...
| ↳ History — calipers | ✅ hover → click → click → click | 🔬 | 🔬 | **Deliberate interaction divergence.** A finger cannot hover, and its ~9 mm contact patch is 360 ms of paper — wider than a QRS — so the web's model measures what you cannot see. Two crosshairs are placed into the visible window the moment the tool opens and are DRAGGED, each by the finger's *delta* (never to its absolute position), so the point stays visible beside the hand. Same Δt / implied-BPM / ΔmV readout, pinned above the sheet, never beside the markers. Precision comes from zooming in — the scale is never rescaled |
| ↳ History — mark / cursor / compare | ✅ | 🔬 | 🔬 | Ported as MODES (one tool at a time; a finger has no modifier keys). Marks: tap to label, tap to edit, drag to move — committed once on release, in-place PATCH. Cursors: tap to drop across every lead, tap to remove, top handle to drag. Compare: all three alignments (beat / fiducial warp / manual), `alignByFiducials` now shared. **Divergence:** the ghost drag is its own mode, because a drag that both pans the paper and moves the ghost cannot be either |
| ↳ History — zoom | ✅ Ctrl+wheel + − / + | 🔬 | 🔬 | **− / + and Fit only; no pinch.** Zoom re-renders six tiled vector sheets, which is not a 60 fps operation on the JS thread, and a pinch that stutters reads as a broken control. The web's own buttons do the same job in the same discrete steps. Pinch with a deferred re-render is the obvious follow-up |
...
| `onStartShouldSetPanResponder: () => true` on a wrapper containing a `Pressable` | The wrapper takes the responder on touch-DOWN and the child's `onPress` never fires: a marker that drags perfectly and cannot be opened | Claim on **move** only. `onStartShouldSetPanResponder: () => false` when anything inside must stay tappable |
...
- **Scan History has never been touched on a device.** Everything in it is
  gesture work — dragged calipers, tap-to-mark, a tiled sheet that must not
  show its seams — and `tsc`, `expo export` and `expo-doctor` are all blind to
  every one of those. It is the largest block of 🔬 in this file.
