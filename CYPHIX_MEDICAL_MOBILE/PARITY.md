...
- **Scan History's v0.15.0 was device-tested and six things were wrong**, one
  of them being that no handle on the sheet was draggable at all. v0.16.0 fixes
  them; the fixes themselves have **not** been on a device yet. In particular
  the touch-down + `scrollEnabled` claim, the tap-vs-drag travel threshold
  (6 pt — possibly too tight for a shaky hand), and the landscape rotation
  round-trip are all things only a phone can confirm.
